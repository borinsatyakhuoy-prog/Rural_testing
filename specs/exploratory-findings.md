...
## Cycle 4 findings (2026-08-06) — live re-exploration for new test coverage

**Scope:** targeted exploration via Playwright MCP browser tools to find genuine coverage gaps not yet automated, per the existing Test Coverage Analysis in `test-results/Report.md` §7 and `user-stories/SCRUM.md`'s Technical Notes. Full detail and resulting automation in `test-results/Report.md` §11.

5. **Every "Explore Lodge" CTA in the customer dashboard opens a new browser tab, unlike every other in-shell link (new finding).** Clicked all 3 instances on `/en/customer/dashboard` (sidebar link, greeting-header button, and the "No bookings found" empty-state button) — each opened `/en` in a brand-new tab via `context.waitForEvent('page')`, confirmed 3 times in the same session, while `Booking`/`Notifications`/`Wishlist` sidebar links all navigate in the same tab as expected. Screenshots not captured (browser-tab behavior doesn't show in a single-page screenshot); reproduced via the MCP tool's own "Open tabs" listing, which grew by one new tab per click. Documented as *current* behavior (regression lock-in test, same pattern as the Offers copy bug) rather than a hard failure, since it could be an intentional "exit the authenticated shell" design choice — but it reads as a genuine UX inconsistency worth a product decision, since repeated clicks leave a customer with accumulating orphaned dashboard tabs. Coverage: `tests/rural-lodge-test/customer-booking/009_explore-lodge-cta-opens-new-tab.spec.ts`.
6. **Owner Lodges search box's transient "0 loges" flash reproduces on more search terms than previously confirmed.** The Status-filter test (`lodge-owner-crud/002`) already documented this race for a checkbox filter; this session reconfirmed it for the free-text search box too — searching "QA" against the `TEST_USER_EMAIL` account's 76 lodges showed **0 loges** immediately after typing, then **34 loges** once polled a few seconds later (real matches, e.g. `QA_CRUD_1785922085349_e05o`). Not a defect — genuine staging latency on the `propertyListing.getPropertyList` query, consistent with the already-documented pattern of transient 502s/slow table loads. New automated coverage (`lodge-owner-crud/003_lodges-list-search-filters-by-name.spec.ts`) polls past this flash rather than asserting immediately, the same technique `002` already uses.
7. **Browser Back/Forward button behavior confirmed correct (not a defect) — closes a real coverage gap.** `user-stories/SCRUM.md`'s Technical Notes explicitly call for "Test navigation flow and back button behavior," which no prior script exercised (only in-app link clicks had been automated). Manually drove Stay → Offers → Activity, then pressed Back twice and Forward once via the MCP browser tools: history unwound correctly (`/en/activity` → Back → `/en/offers` → Back → `/en` → Forward → `/en/offers`), with the locale segment preserved at every step. New coverage: `tests/rural-lodge-test/navigation/013_browser-back-button-preserves-history.spec.ts`.
